...
using (var dbconnection = new SqlConnection(@"Data source=(localdb)\MSSQLLocalDB;Initial Catalog=EmployeeDb"))
{
    ...
    IEmployeeRepository employeeDapperRepository = new EmployeeDapperRepository(dbconnection);
    try
    {
        var employees = await employeeDapperRepository.GetEmployeesAsync(1, 5, "Id", filterText: "Parm".ToLower());
